...
public interface MealMngmRepository extends JpaRepository<MealMngm, MealMngmId> {
	...
	// 사용자가 직접 클릭한 조식
	// @Query(value = "insert into mealMngm(mem_serial, meal_date, mem_name,
	// staff_nm, brf_meal) " +
	// "value(:mem_serial, :meal_date, :mem_name, :staff_nm , :brf_meal)",
	// nativeQuery = true)
	// MealMngm save(@Param("mem_serial") String mem_serial, @Param("meal_date")
	// String meal_date, @Param("mem_name") String mem_name,
	...
	// c("SELECT new com.child.project.domain.mealDTO(SELECT
	// COUNT(substr(mealDate,1,7)) FROM mealMngm AS B WHERE brfMeal=1 AND
	// substr(A.mealDate,1,7) = substr(B.mealDate,1,7)) AS brfMeal,"
	// + "(SELECT COUNT(substr(mealDate,1,7)) FROM mealMngm AS B WHERE lncMeal=1 AND
	// substr(A.mealDate,1,7) = substr(B.mealDate,1,7)) AS lncMeal, "
	// + "(SELECT COUNT(substr(mealDate,1,7)) FROM mealMngm AS B WHERE dnrMeal=1 AND
	// substr(A.mealDate,1,7) = substr(B.mealDate,1,7)) AS dnrMeal, "
	// + "(SELECT COUNT(substr(mealDate,1,7)) FROM mealMngm AS B WHERE snkMeal=1 AND
	// substr(A.mealDate,1,7) = substr(B.mealDate,1,7)) AS snkMeal "
	// + "FROM mealMngm AS A "

...
public interface ProgramRepository extends JpaRepository<Program, ProgramId> {
    ...
    @Query(value = "select * from program where STR_TO_DATE(:formatedNow, '%Y-%m-%d') between STR_TO_DATE(prg_str, '%Y-%m-%d') and STR_TO_DATE(prg_end, '%Y-%m-%d') "
            + "order by prg_str,prg_end", nativeQuery = true)
    List<Program> selectSlide(@Param("formatedNow") String formatedNow);
    ...
    List<Program> findSearch(@Param("prg_str") String prgStr, @Param("prg_end") String prgEnd,
            @Param("prg_nm") String prgNm, @Param("prg_mngr") String prgMngr,
            @Param("prg_cls") String prgCls);
